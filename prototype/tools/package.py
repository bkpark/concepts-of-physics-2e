...
The four approved repairs are applied, with native selectable MathML. CNX numbering
has been checked at selected reference points; course object numbering remains experimental.
This is not a student edition.</p>
...
pdfs=sorted(out.glob('introduction-to-physics-maintained-*.pdf'))
...
with zipfile.ZipFile(out/'introduction-to-physics-maintained-preview.zip','w',zipfile.ZIP_DEFLATED) as z:
    active=[dist/'index.html']+[p for profile in ('cnx','course') for p in (dist/profile).rglob('*')]
    ...
    z.write(root.parent/'docs/maintained-source-status.md','ASSESSMENT.md')
print(out/'introduction-to-physics-maintained-preview.zip')
