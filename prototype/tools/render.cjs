...
const result={browser:browser.version(),playwright:require('playwright/package.json').version,external_requests:external,pages:[]};
try{
for(const profile of ['cnx','course']){
 ...
 for(const [mid,entry] of Object.entries(ids)){
  ...
  const audit=await page.evaluate(()=>({mathWrappers:document.querySelectorAll('[data-math-key]').length,renderedMath:document.querySelectorAll('mjx-container').length,mathErrors:[...document.querySelectorAll('[data-mml-node="merror"]')].map(e=>({key:e.closest('[data-math-key]')?.dataset.mathKey,message:e.textContent})),unresolved:[...document.querySelectorAll('.math-issue')].map(e=>e.dataset.mathKey),brokenImages:[...document.images].filter(i=>!i.complete||!i.naturalWidth).map(i=>i.src),overflow:[...document.querySelectorAll('mjx-container,table,img')].filter(e=>e.getBoundingClientRect().right>document.querySelector('main').getBoundingClientRect().right+2).map(e=>({tag:e.tagName,key:e.closest('[data-math-key]')?.dataset.mathKey})),duplicateIds:[...document.querySelectorAll('[id]')].map(e=>e.id).filter((x,i,a)=>a.indexOf(x)!==i)}));
 ...
 await page.pdf({path:path.join(out,`introduction-to-physics-prototype-${profile}.pdf`),format:'Letter',preferCSSPageSize:true,printBackground:true,displayHeaderFooter:true,headerTemplate:'<div></div>',footerTemplate:'<div style="font-size:8px;width:100%;text-align:center;color:#54656c">Introduction to Physics · Fidelity prototype · <span class="pageNumber"></span> / <span class="totalPages"></span></div>',tagged:true,outline:true});
 await page.close();
}
