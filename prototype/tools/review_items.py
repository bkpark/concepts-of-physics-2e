...
items=[];maths=[i for i in b['issues'] if i['kind']=='unresolved-math'];refs=[i for i in b['issues'] if i['kind'] in ('reference-outside-book','source-reference-unresolved')]
...
for kind,findings in [('M',maths),('X',refs)]:
 for number,item in enumerate(findings,1):
  ...
  if kind=='M':
   ...
   body+='<p>Missing destination: <code>'+esc(destination)+'</code>. '+('The referenced module is outside the recovered book.' if item['kind']=='reference-outside-book' else 'The target ID is absent from this module.')+'</p><div class="context">'+html+'</div>'
