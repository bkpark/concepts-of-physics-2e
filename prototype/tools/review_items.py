...
items=[];maths=[i for i in b['issues'] if i['kind']=='unresolved-math'];refs=[i for i in json.loads((ROOT/'reports/course-full-findings.json').read_text(encoding='utf-8'))['findings'] if i['kind'] in ('reference-outside-book','source-reference-unresolved')] # Freeze review IDs across repairs.
...
for kind,findings in [('M',maths),('X',refs)]:
 for number,item in enumerate(findings,1):
  ...
  elif applied and applied.get('change_kind')=='replace-prose':
  ...
  exercise=next((p for p in ancestors if local(p)=='exercise'),None)
