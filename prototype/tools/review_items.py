 ...
 for number,item in enumerate(findings,1):
  ...
  if change_path.exists() and (kind=='X' or json.loads(change_path.read_text(encoding='utf-8'))['status']=='applied-author-directed'):
   ...
   if change['status']=='applied-author-directed':
    applied=change
    if applied.get('change_kind') not in ('replace-prose','replace-math'):item={**item,'document':applied['target_document'],'target':applied['target_id']}
   else:
    assert change['status']=='preview-awaiting-author-confirmation'
    wording_proposal=change
  if applied and applied.get('change_kind') in ('replace-prose','replace-math'):
  ...
  if applied and applied.get('change_kind') in ('replace-prose','replace-math'):context=e
  source=e
  ...
  if kind=='M':
   ...
   body+='<p>'+('Original finding: ' if applied else '')+esc(explanation)+'</p><h3>Current build context</h3><div class="context">'+html+'</div>'
