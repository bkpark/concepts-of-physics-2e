...
page+='<section id="SR01"><h2>SR01 — Emission is not reception</h2><p>The current passage says A receives both flashes together. In the stated setup, A receives the right-hand flash first. Both observers agree on that order of reception at A; what differs by frame is whether the two remote emission events were simultaneous.</p><p>The upstream correction improves the result, but still describes the train moving right while discussing its own rest frame. Its summary also needs the light-travel-time qualification. The proposal below addresses the complete explanation, rather than substituting just one sentence.</p><p><a href="'+live(mid,sections[mid]['slug'],'fs-id3155062')+'">Read the current textbook passage</a></p>'
...
page+='<div class="columns"><div class="box"><h3>Current 1.0 wording</h3>'+''.join(review_markup(x['before_xml']) for x in item['changes'])+'</div><div class="box"><h3>Proposed 1.1 wording</h3>'+''.join(review_markup(x['after_xml']) for x in item['changes'])+'</div></div>' 
page+='<figure><img src="train-flashes.jpg" alt="Existing two-panel train-and-flash-lamps figure"><figcaption><b>Existing artwork; proposed caption:</b> '+esc(item['figure']['proposed_caption'])+'</figcaption></figure><details><summary>Proposed image description</summary><p>'+esc(item['figure']['proposed_alt'])+'</p></details>'
page+='<details><summary>Physics check and exact scope</summary><p>In B’s frame let the emission positions be −ℓ and +ℓ at t = 0, with A at x = vt and 0 &lt; v &lt; c. Reception at A occurs at t(right) = ℓ/(c+v) and t(left) = ℓ/(c−v), so the right flash arrives first. The Lorentz transformation gives t′(right emission) − t′(left emission) = −2γvℓ/c²: the right emission is earlier in A’s frame. Both reception events occur at A’s position, so their order cannot reverse between these frames.</p><p>This proposes six paragraph replacements plus caption and alt-text changes. Existing section, paragraph, figure and media IDs and the figure cross-reference remain. The artwork is unchanged. The isolated c symbol in the current explanatory paragraph becomes “the same speed”; no equation outside this passage is changed.</p></details></section>'
