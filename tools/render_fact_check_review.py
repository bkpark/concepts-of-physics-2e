...
items = packet['items']
applied = all(i['status'] == 'applied-author-directed' for i in items)
patterns = {
...
limits = [
    ...
    ('All 35 approved items were applied to the maintained 1.1 preview on 2026-09-20. Comparisons preserve the reviewed snapshots and may show only the scoped edit; the textbook context links show the combined result.' if applied else 'All proposals are unapplied. Full XML snippets are review snapshots; later edits must preserve canonical byte structure and use the editorial ledger.'),
...
page=['<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>1.1 focused fact-check review</title><style>'+css+'</style></head><body>',
      ...
      '<p class="notice">'+str(summary['proposed_corrections'])+' corrections and '+str(summary['author_discussion'])+' discussion items. <strong>All 35 approved items are applied to the maintained 1.1 preview.</strong> Published 1.0 is unchanged. Additional factual leads remain open. Review date: '+packet['date']+'.</p>',
      ...
      '<p><a href="#corrections">Reviewed corrections</a> · <a href="#discussion">Reviewed discussion items</a> · <a href="#limits">Coverage and limits</a> · <a href="review.md">Readable Markdown copy</a></p>']
if stale and not applied:page.append('<p class="hold">Canonical source has changed since this snapshot: '+', '.join(stale)+'. Reconcile before applying.</p>')
md=['# Focused fact-check for 1.1','', 'Review snapshot '+packet['date']+'. Approved cycle applied to the maintained preview; additional leads remain open.','']
for category,label in [('corrections','Reviewed corrections'),('discussion','Reviewed discussion items')]:
    ...
    for i in group:
        ...
        if i['edits']:
            page.append('<details class="edits"><summary>Recorded substitutions</summary><ul>'+''.join('<li><del>'+esc(e['before'])+'</del> → <ins>'+esc(e['after'] or '[delete]')+'</ins></li>' for e in i['edits'])+'</ul></details>')
        page.append('<div class="cols"><div class="box"><h3>Before this review cycle</h3>'+render(i['before_xml'],i['module'])+'</div><div class="box"><h3>'+('Approved change — applied' if i['proposed_xml'] else 'Decision needed')+'</h3>'+ (render(i.get('preview_xml', i['proposed_xml']),i['module']) if i['proposed_xml'] else '<p>'+esc(i['reason'])+'</p>')+'</div></div>')
        if i.get('structural_proposal'):
            page.append('<p class="notice"><strong>Approved order change — applied:</strong> '+esc(i['structural_proposal']['description'])+'</p>')
        for passage in i.get('related_passages', []):
            ...
            page.append('<div class="cols"><div class="box"><h3>Before this review cycle</h3>'+render(passage['before_xml'],i['module'])+'</div><div class="box"><h3>Approved change — applied</h3>'+render(passage.get('preview_xml', passage['proposed_xml']),i['module'])+'</div></div>')
            page.append('<details class="edits"><summary>Recorded substitutions for this passage</summary><ul>'+''.join('<li><del>'+esc(e['before'])+'</del> → <ins>'+esc(e['after'] or '[delete]')+'</ins></li>' for e in passage['edits'])+'</ul></details>')
        page.append('<ul>'+''.join('<li><a href="'+esc(s['url'])+'">'+esc(s['title'])+'</a></li>' for s in i['sources'])+'</ul></section>')
        md+=['## '+i['id']+' — '+i['title'],'',i['section_title']+' · '+i['status'],'',i['reason'],'','**Before this review cycle:** '+re.sub(r'\s+',' ',plain(E.fromstring(i['before_xml']))),'']
        if i['proposed_xml']:md+=['**Approved change — applied:** '+re.sub(r'\s+',' ',plain(E.fromstring(i.get('preview_xml', i['proposed_xml'])))),'']
        if i.get('structural_proposal'):md+=['**Approved order change — applied:** '+i['structural_proposal']['description'], '']
        for passage in i.get('related_passages', []):
            md += ['### '+passage['title'], '',
                   '**Before this review cycle:** '+re.sub(r'\s+',' ',plain(E.fromstring(passage['before_xml']))).strip(), '',
                   '**Approved change — applied:** '+re.sub(r'\s+',' ',plain(E.fromstring(passage.get('preview_xml', passage['proposed_xml'])))).strip(), '']
