...
page+='<p class="notice">Reviewed '+str(summary['section_count'])+' sections, including the remaining Kinematics differences, all of Dynamics, and all five Unit 2 chapters. Applied '+str(len(batch['changes']))+' scoped correction records. The plasma expansion is held for your decision; no chapter or section restructuring was performed. This is a development draft. The full-book backport and release PDF checks remain unfinished.</p><p><a href="../../course-full/">Current 1.1 development textbook</a> · <a href="#plasma">Held coverage decision</a> · <a href="#retained">Retained differences and limitations</a></p>'
...
for x in batch['changes']:
    r=load(x['record']);sec=sections[x['module']];anchor=x['module']+'--'+x['source_id'].encode().hex()
    ...
    page+='</div></section>'
page+='<section id="plasma"><h2>Held: plasma in “What Is a Fluid?”</h2><p>Upstream expands the three-phase overview to include plasma, coordinating three paragraphs, the figure caption and a fourth panel. This adds coverage; it is not needed to repair the existing solids/liquids/gases explanation. Recommendation: defer this expansion to the broader content revision. No part of it has been imported.</p><div class="cols">'
