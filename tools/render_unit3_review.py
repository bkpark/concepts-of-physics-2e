...
page+='<p>Thermal physics, electricity, magnetism, and light — 49 sections.</p><p class="notice">'+str(len(batch['changes']))+' scoped correction records applied to the maintained development draft. Six items below are held for your review and have not been applied. Existing section identities, numbering, organization and exercise placement are preserved. Published 1.0 is unchanged.</p>'
...
page+='<h2 id="held">Held for your review — unchanged in textbook</h2><ul>'+''.join('<li><a href="#'+h['id']+'">'+h['id']+' — '+esc(h['title'])+'</a></li>' for h in holds)+'</ul>'
for h in holds:
 page+='<section id="'+h['id']+'"><h2>'+h['id']+' — '+esc(h['title'])+'</h2><p>'+esc(h['reason'])+'</p><p class="held">'+esc(h['recommendation'])+'</p><p><a href="'+link(h['module'],h['source_id'])+'">View current passage in textbook</a></p><div class="cols">'
 for side,label in [('before','Current textbook — unchanged'),('upstream','Pinned upstream — not applied')]:
  body=render(h[side],h['module']) if h.get(side) else 'No corresponding upstream block.'
  if h['id']=='U3-H05':
   src=ROOT/('maintained/media/Figure_24_01_03.jpg' if side=='before' else 'tmp/unit3-media/Figure_24_01_03.jpg');name='generator-'+side+'.jpg';shutil.copyfile(src,OUT/name);body='<img src="'+name+'" alt="'+esc(label+' generator-current diagram')+'">'+body
