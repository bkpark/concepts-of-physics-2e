...
summary={'upstream_commit':ref['commit'],'archive_sha256':ref['archive']['sha256'],'upstream_collection_modules':len(up),
         ...
         'status':('Partial review: unresolved comparison differences remain.' if any(c['status'] in ('unreviewed-difference','held-for-author-review') for c in all_changes) else 'Comparison review complete: all reported differences have recorded dispositions. Intentional deferrals and release verification remain separate.'),
...
for row in rows:
    ...
    page+='<p>Compared differences with recorded review decisions. Neither column is automatically preferred. Math matches use owner ID and ordinal, which can shift after structural edits.</p>'
