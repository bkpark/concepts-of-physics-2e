...
(out/'index.html').write_text('<!doctype html><html lang="en"><meta charset="utf-8"><title>Radiation section previews</title><link rel="stylesheet" href="style.css"><main><h1>Radiation section reading previews</h1><p>Cycle-2 changes overlaid for review; canonical source unchanged.</p><ul>'+''.join('<li><a href="sections/'+ns['registry'][m]['candidate_slug']+'/index.html">'+ns['registry'][m]['title']+'</a></li>' for m in selected)+'</ul></main></html>',encoding='utf-8')
(out/'math-sources.json').write_text(json.dumps(ns['math_sources'],ensure_ascii=False),encoding='utf-8')
report={'modules':selected,'overlays':changes,'issues':ns['issues'],'source_sha256':{m:hashlib.sha256((ROOT/ns['registry'][m]['source']).read_bytes()).hexdigest() for m in selected}}
